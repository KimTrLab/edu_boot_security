...
public class FileDataUtil {
	 ...
	 * @throws IOException 
	 */
	public String[] fileUpload(List<MultipartFile> file) throws IOException {
		String[] files = new String[file.size()];
		for(int i=0; i < file.size(); i++) {
			MultipartFile m = file.get(i);
			if(m.getOriginalFilename()!="") {  // 실제 file객체가 존재한다면
				String originalName = m.getOriginalFilename();//확장자가져오기 위해서 전체파일명을 가져옴.
